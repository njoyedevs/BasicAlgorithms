...
function symmetricDifferences(numbersA, numbersB) {
    ...
    for (let i = 0; i < numbersA.length; i++) {
        if (!numbersB.includes(numbersA[i]) && !newArray.includes(numbersA[i])) {
            newArray.push(numbersA[i])
        }
        if (!numbersA.includes(numbersB[i]) && !newArray.includes(numbersB[i])) {
            newArray.push(numbersB[i])
        }
    }
