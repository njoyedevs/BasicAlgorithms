...
function functionalFilterByKey(items, searchFor, searchBy, searchMethod) {
    ...
    const method = (value, searchMethod, searchFor) => {
        const bool = value[searchMethod](searchFor);
        return bool
    }
    ...
    for (const item of items) {
        ...
        for (const [key, value] of Object.entries(item)) {
            ...
            if (key === searchBy) {
                // console.log(value.includes(searchFor))
                // if (value.startsWith(searchFor)) {
                //     newArray.push(item)
                // }
                if (method(value, searchMethod, searchFor)) {
                    newArray.push(item)
                }
            }
        }
...
// Options include: startsWith, endsWith, includes
const searchMethod = "startsWith"
console.log(`\nTest1`)
console.log(functionalFilterByKey(people, searchFor1, searchBy1, searchMethod))
console.log(`\nTest2`)
console.log(functionalFilterByKey(people, searchFor2, searchBy2, searchMethod))
console.log(`\nTest3`)
console.log(functionalFilterByKey(people, searchFor3, searchBy3, searchMethod))